...
position_metrics = {
    ...
    "Left Back": {"metrics": ["Passing%","Pr. Pass% Dif.","Successful Crosses","Crossing%",
    ...
    "Right Back": {"metrics": ["Passing%","Pr. Pass% Dif.","Successful Crosses","Crossing%",
                               "Deep Progressions","Successful Dribbles","Turnovers","OBV","Pass OBV",
                               "Defensive Actions","Aerial Win%","PAdj Pressures","PAdj Tack&Int",
                               "Dribbles Stopped%","Aggressive Actions","Player Season Ball Recoveries 90"]},
    ...
    "Number 8": {"metrics": ["xGBuildup","xG Assisted","Shots","xG","NP Goals","Passing%",
                             ...
                             "Aggressive Actions"]},
    "Left Wing": {"metrics": ["xG","Shots","xG/Shot","Touches In Box","OP xG Assisted","NP Goals",
    ...
    "Right Wing": {"metrics": ["xG","Shots","xG/Shot","Touches In Box","OP xG Assisted","NP Goals",
                               "OP Passes Into Box","Successful Box Cross%","Passing%","Successful Dribbles",
                               "Turnovers","OBV","D&C OBV","Fouls Won","Deep Progressions",
                               "Player Season Fhalf Pressures 90"]},
    "Striker": {"metrics": ["Aggressive Actions","NP Goals","xG","Shots","xG/Shot","Goal Conversion%",
...
def load_statsbomb(path: Path) -> pd.DataFrame:
    if path.is_file():
        ...
        return pd.concat([load_one_file(f) for f in files], ignore_index=True, sort=False)
...
def compute_rankings(df_all: pd.DataFrame, min_minutes: int = 600) -> pd.DataFrame:
    ...
    def _minmax_score(v, lo, hi):
        if pd.isna(v) or pd.isna(lo) or pd.isna(hi) or hi <= lo:
            return 50.0
        return np.clip((v - lo) / (hi - lo) * 100.0, 0, 100)

    df_all["Score (0–100)"] = [
        _minmax_score(v, lo, hi) for v, lo, hi in zip(df_all["Weighted Z Score"], df_all["_scale_min"], df_all["_scale_max"])
    ]
...
try:
    ...
    if "Name" in df_all.columns: df_all.rename(columns={"Name":"Player"}, inplace=True)
    if "Primary Position" in df_all.columns: df_all.rename(columns={"Primary Position":"Position"}, inplace=True)

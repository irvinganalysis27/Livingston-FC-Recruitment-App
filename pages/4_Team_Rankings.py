...
def map_first_position_to_group(pos: str) -> str:
    return RAW_TO_SIX.get(_clean_pos_token(pos), None)
...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    # Rename basics
    if "Name" in df.columns: df.rename(columns={"Name":"Player"}, inplace=True)
    if "Primary Position" in df.columns: df.rename(columns={"Primary Position":"Position"}, inplace=True)
    if "Minutes" in df.columns: df.rename(columns={"Minutes":"Minutes played"}, inplace=True)

    # Add age
    if "Birth Date" in df.columns:
        today = datetime.today()
        df["Age"] = pd.to_datetime(df["Birth Date"], errors="coerce").apply(
            lambda dob: today.year - dob.year - ((today.month, today.day)<(dob.month,dob.day)) if pd.notna(dob) else np.nan
        )
    ...
    # Duplicate generic CMs into 6 & 8
...
# ============================================================
# Ranking logic (full Radar page version)
# ============================================================
...
def compute_rankings(df_all: pd.DataFrame, min_minutes: int=400) -> pd.DataFrame:
    ...
    def _minmax(val,lo,hi):
        if pd.isna(val) or pd.isna(lo) or pd.isna(hi) or hi<=lo: return 50.0
        return np.clip((val-lo)/(hi-lo)*100.0,0.0,100.0)

    df_all["Score (0–100)"]=[_minmax(v,lo,hi) for v,lo,hi in zip(df_all["Weighted Z Score"],df_all["_scale_min"],df_all["_scale_max"])]
...
def plot_team_433(df, club_name, league_name):
    formation_roles={"GK":["Goalkeeper"],"LB":["Full Back"],"LCB":["Centre Back"],"RCB":["Centre Back"],"RB":["Full Back"],
                     "CDM":["Number 6"],"LCM":["Number 8"],"RCM":["Number 8"],"LW":["Winger"],"RW":["Winger"],"ST":["Striker"]}
    ...
    ax.set_facecolor("white"); ax.set_xlim(0,100); ax.set_ylim(0,100); ax.axis("off")
    ax.set_title(f"{club_name} ({league_name})",color="black",fontsize=16,weight="bold")

    coords={"GK":(50,5),"LB":(10,25),"LCB":(37,20),"RCB":(63,20),"RB":(90,25),
            "CDM":(50,40),"LCM":(30,55),"RCM":(70,55),

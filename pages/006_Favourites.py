# ============================================================
# ⭐ WATCH LIST PAGE (with radar-identical mapping + Shadow Team add + persistent filters)
# ============================================================
...
from lib.ui_wrappers import ui_multiselect, ui_toggle, ui_selectbox  # ✅ NEW WRAPPERS
...
from lib.favourites_repo import list_favourites, upsert_favourite, delete_favourite, hide_favourite
...
RAW_TO_GROUP = {
    ...
    "CENTREMIDFIELDER": "Number 8", "LEFTCENTREMIDFIELDER": "Number 8",
    "RIGHTCENTREMIDFIELDER": "Number 8", "CENTREATTACKINGMIDFIELDER": "Number 8",
    ...
    "CENTREFORWARD": "Striker", "LEFTCENTREFORWARD": "Striker",
    "RIGHTCENTREFORWARD": "Striker", "SECONDSTRIKER": "Striker", "10": "Striker",
}
...
with top_c1:
    show_hidden = ui_toggle("Show hidden players", key="show_hidden", default=False)
with top_c2:
    st.caption("Filter by colour/status or hide players below.")

selected_statuses = ui_multiselect(
    "Filter by Status",
    options=COLOUR_CHOICES,
    key="status_filter",
    default=["🟣 Needs Checked"],
...
available_groups = [g for g in SIX_GROUPS if g in {r["mapped_position"] for r in rows_all if r.get("mapped_position")}]

selected_groups = ui_multiselect(
...
if not rows:
    ...
    for row in rows:
        with st.container(border=True):
            ...
            # Comments & Actions remain identical below...
            # (no logic changed — only filters made persistent)

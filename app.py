...
DATA_PATH = (APP_DIR / "statsbombdata.xlsx")
...
position_mapping = {
    ...
    "Striker Old": "Striker",
    "Striker New": "Striker",
    "Full Back Old": "Full Back",
    "Full Back New": "Full Back",
    "Centre Back Old": "Centre Back",
    "Centre Back New": "Centre Back",
    "Number 6 Old": "Number 6",
    "Number 6 New": "Number 6",
    "Number 8 Old": "Number 8",
    "Number 8 New": "Number 8",
}
main_position = position_mapping.get(selected_position_template, next(iter(position_metrics[selected_position_template]["groups"].values())))
if main_position not in ["Goalkeeper", "Full Back", "Centre Back", "Number 6", "Number 8", "Winger", "Striker"]:
...
if main_position:
    df_all_600 = df_all_600[df_all_600[pos_col] == main_position].copy()
percentile_df_globalpos_all = pd.DataFrame(index=df_all_600.index, columns=metrics, dtype=float)
for m in metrics:
    df_all_600[m] = pd.to_numeric(df_all_600[m], errors="coerce").fillna(0)
    try:
        percentile_df_globalpos_all[m] = (
            df_all_600.groupby(pos_col, group_keys=False)[m]
                      .apply(lambda s: pct_rank(s, lower_is_better=(m in LOWER_IS_BETTER)))
        )
    except Exception as e:
        print(f"[DEBUG] Global percentile calc failed for {m}: {e}")
        percentile_df_globalpos_all[m] = 50.0
percentile_df_globalpos = percentile_df_globalpos_all.reindex(df_all.index, fill_value=50.0).round(1)

# Raw Z-Scores for RANKING
raw_z_all = pd.DataFrame(index=df_all_600.index, columns=metrics, dtype=float)
for m in metrics:
    df_all_600[m] = pd.to_numeric(df_all_600[m], errors="coerce").fillna(0)
    group_stats = df_all_600.groupby(pos_col)[m].agg(['mean', 'std']).fillna(0)
    z_per_group = df_all_600.groupby(pos_col)[m].transform(lambda x: (x - x.mean()) / x.std() if x.std() != 0 else 0)
...
def plot_radial_bar_grouped(player_name, plot_data, metric_groups, group_colors=None):
    ...
    if not all(c in row.columns for c in pct_cols):
        st.error(f"Missing percentile columns for {player_name}: {pct_cols}")
        return
    pct_data = row[pct_cols]
    try:
        ...
        st.error(f"Invalid percentile data for {player_name}. Skipping plot.")
        return
    n = len(sel_metrics)
    if len(pct_vals) != n:
        print(f"[DEBUG] Mismatch: Expected {n} metrics, got {len(pct_vals)} for {player_name}")
        st.error(f"Metric count mismatch for {player_name}. Expected {n}, got {len(pct_vals)}.")
        return
    if not np.all(np.isfinite(pct_vals)):
        print(f"[DEBUG] Non-finite values detected for {player_name}: {pct_vals}")
        st.warning(f"Invalid percentile values for {player_name}. Using neutral percentiles (50).")
        pct_vals = np.full(n, 50.0)
